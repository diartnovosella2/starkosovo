    ...
    <div class="content__left <?= $styleClass;?> content__left d-flex flex-column <?= $toggleFooter; ?> n bg_<?= $bgColor; ?>">
        <?php if($chooseStyle === 'Get Started')  { ?>
            ...
            <?php } else if($chooseStyle === 'Success Story') { ?>
                <h3 class="content__left__topdesc__<?= $styleClassTitle; ?> d-flex justify-content-end pl-2 pl-lg-5 pr-3 mb-0">SUCCESS STORY</h3>
        ...
        <?php } else {?>
            <div class="content__left__description content__left__description__register px-2 px-lg-5 <?= $toggleMargin; ?>">
                <h2 class="content__left__title__<?= $styleClass;?> pr-0 pr-lg-5 mb-2 mb-lg-3"><?= $data['title']; ?></h2>
                <?= $data['content_description'];?>
                <?php if( strlen($data['register_button_url']) !== 0) {?>
                    <button class="apply__button mt-3"> 
                        <a href="<?= $data['register_button_url'];?>"><?= $data['register_button_text'];?></a>
                    </button>
